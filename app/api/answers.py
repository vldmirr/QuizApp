...
def create_answer(question_id: int = Path(..., gt=0), answer: AnswerCreate = ...,db: Session = Depends(get_db)):
    ...
    question = serviceQ.get_question(db, question_id=question_id)
    if not question:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Question not found")
    
    return serviceA.create_answer(db=db, answer=answer, question_id=question_id)
...
def get_answer(answer_id: int, db: Session = Depends(get_db)):
    ...
    db_answer = serviceA.get_answer(db, answer_id=answer_id)
...
def delete_answer(answer_id: int, db: Session = Depends(get_db)):
    """Удалить ответ"""
    success = serviceA.delete_answer(db, answer_id=answer_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Answer not found"
