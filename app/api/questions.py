...
def get_questions(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    return crud_questions.get_questions(db, skip=skip, limit=limit)

@router.post("/", response_model=QuestionResponse, status_code=status.HTTP_201_CREATED)
def create_question(question: QuestionCreate, db: Session = Depends(get_db)):
    """Создать новый вопрос"""
    return crud_questions.create_question(db=db, question=question)

@router.get("/{question_id}", response_model=QuestionWithAnswers)
def get_question(question_id: int, db: Session = Depends(get_db)):
    """Получить вопрос и все ответы на него"""
    db_question = crud_questions.get_question_with_answers(db, question_id=question_id)
...
def delete_question(question_id: int, db: Session = Depends(get_db)):
    ...
    success = crud_questions.delete_question(db, question_id=question_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Question not found"
